...
Array.prototype.addIfNotExist = function (val) {
...
function MotionEvent(rawEv) {
  ...
  if (rawEv.type == 'mouseout') {
    if (
      rawEv.x <= 0 ||
      rawEv.x >= window.innerWidth ||
      rawEv.y <= 0 ||
      rawEv.y >= window.innerHeight
    ) {
      action = 3
    } else {
      action = 2
...
function ViewGroup() {
  ...
  this.addView = function (v, indexOrParams, params) {
    if (v.getParent() != null) {
      console.log(
        'IllegalStateException: ' + v.getTag() + ' 只能拥有一个父节点'
      )
      return
    }
    v.setParent(this)
...
function setContentView(view, htmlnode) {
  ...
  var divCss =
    'position:absolute; box-sizing:border-box; overflow:hidden; text-overflow:ellipsis;'
  css.innerHTML =
    "body > div[tag='decorview']{" +
    divCss +
    "} body > div[tag='decorview'] div{" +
    divCss +
    '} ' +
...
document.write(
  unescape(
    "%3Cspan id='cnzz_stat_icon_1256652931'%3E%3C/span%3E%3Cscript src='" +
      cnzz_protocol +
      "s4.cnzz.com/z_stat.php%3Fid%3D1256652931' type='text/javascript'%3E%3C/script%3E"
  )
)
///* statistics code end */
...
function forceReLayout() {
  ...
  if (mRootNode == document.body) {
    ...
    if (
      mLastOffset == null ||
      offset.width != mLastOffset.width ||
